...
helpers do
  ...
  def gen_m3u8
    if settings.job.video
      ...
      cmd_step_3  = "#{settings.ffmpeg_path} -headers \"$cookie\" -i \"#{Base64.decode64(settings.job.video)}\" -vcodec copy -vbsf h264_mp4toannexb -map 0:0 -acodec aac -strict experimental -ac 2 -ab 160k -ar 48000 -async 1 -map 0:1  -f segment -segment_time 5 -flags +loop -flags2 +fast -segment_list movie.m3u8 -segment_format mpegts -segment_list_flags +live stream%05d.ts"
